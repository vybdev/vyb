...
- [ ] Introduce new request structs in `llm/payload` and add unit tests verifying JSON marshalling round-trips.
- [ ] Update the public `llm` provider interface in `llm/dispatcher.go` to accept the new structs. Add compile-time tests catching signature mismatches.
