...
- [ ] Evaluate all the information you have so far and elaborate a plan with all the coding steps needed to take the system from its current state to what is described in the section "What the solution looks like". The plan should consist of individual atomic changes to the code. Each change should leave the code in a consistent state, compiling and passing tests. Avoid leaving tests to the end of the list, and instead, add them alongside the new and updated code you are introducing. Each step should be added as an individual item in the "TODO List" section of this document;  